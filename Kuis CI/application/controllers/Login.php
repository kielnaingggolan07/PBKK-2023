...
class Login extends CI_Controller {
	...
	public function daftar(){
		...
		if ($this->form_validation->run() == false) {
			...
			$this->session->set_flashdata('message', 'swal("Success", "Successfully Registered. Welcome to Go-Bus!", "success");');
    		redirect('login');
		}
	...
	Private function _sendmail($token='',$type=''){
        ...
        $this->email->from('Go-Bus');
        $this->email->to($this->input->post('email'));
        // $this->email->attach('https://masrud.com/content/images/20181215150137-codeigniter-smtp-gmail.png');
        if ($type == 'verify') {
        	$this->email->subject('Account verify Go-Bus');
        	...
        	$this->email->subject('Go-Bus Ticket Reset Account');
